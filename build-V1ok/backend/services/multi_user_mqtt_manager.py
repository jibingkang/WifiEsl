    ...
    async def start(self) -> bool:
        ...
        try:
            ...
            if broker_url.startswith("mqtts://"):
                ...
                use_tls = True
            elif broker_url.startswith("ssl://"):
            ...
            if not use_tls:
                try:
                    if ":" in broker_url:
                        port_str = broker_url.rsplit(":", 1)[1]
                        if int(port_str) == 8883:
                            use_tls = True
                except:
                    pass
            ...
            logger.info(f"[MQTT连接信息] 解析后的Broker地址: {self.broker_host}:{self.broker_port}{tls_label}")
            ...
            # ⭐ 端口8883或mqtts://协议：启用TLS加密
            if use_tls:
                self.client.tls_set()
                logger.info(f"[MQTT连接信息] 已启用TLS加密 (端口8883/mqtts协议)")
